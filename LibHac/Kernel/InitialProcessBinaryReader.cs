...
public class InitialProcessBinaryReader : IDisposable
{
        ...
        ref readonly SharedRef<IStorage> iniStorage, int processCount)
    {
        ...
        (int offset, int size)[] offsets = new (int offset, int size)[processCount];
        int offset = Unsafe.SizeOf<IniHeader>();
        using KipReader kipReader = new();

        for (int i = 0; i < processCount; i++)
        {
            using SharedRef<IStorage> kipStorage = new(new SubStorage(in iniStorage, offset, iniStorageSize - offset));
