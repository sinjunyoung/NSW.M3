...
public class SdCardManager : IStorageDeviceManager, IStorageDeviceOperator, ISdmmcDeviceManager
{
    ...
    {
        SdCardManager manager = new(sdmmc);

        using SharedRef<SdCardManager> sharedManager = new(manager);
    ...
    public Result Lock(ref UniqueLockRef<SdkMutexType> outLock, SdmmcHandle handle)
    {
        ...
        using UniqueLockRef<SdkMutexType> scopedLock = new(ref _mutex);
    ...
    {
        SdCardManagerOperationIdValue operation = (SdCardManagerOperationIdValue)operationId;
