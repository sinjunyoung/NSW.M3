...
internal class SdCardStorageDevice : SdmmcStorageInterfaceAdapter, IStorageDevice
{
    ...
    {
        SdCardStorageDevice device = new(in manager, handle, sdmmc);

        using SharedRef<SdCardStorageDevice> sharedDevice = new(device);
    ...
    public Result OpenOperator(ref SharedRef<IStorageDeviceOperator> outDeviceOperator)
    {
        ...
        using SharedRef<SdCardDeviceOperator> deviceOperator = new(new SdCardDeviceOperator(in storageDevice, _sdmmc));
