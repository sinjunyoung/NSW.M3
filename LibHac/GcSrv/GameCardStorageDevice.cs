...
internal class GameCardStorageDevice : GameCardStorageInterfaceAdapter, IStorageDevice
{
    ...
    {
        GameCardStorageDevice storageDevice = new(gc, in manager, in baseStorage, handle);

        using SharedRef<GameCardStorageDevice> sharedStorageDevice = new(storageDevice);
    ...
    public static SharedRef<GameCardStorageDevice> CreateShared(IGcApi gc, ref readonly SharedRef<IGameCardManager> manager,
        ...
        ReadOnlySpan<byte> cardDeviceId, ReadOnlySpan<byte> cardImageHash)
    {
        GameCardStorageDevice storageDevice = new(gc, in manager, in baseStorage, handle, isSecure, cardDeviceId,
            cardImageHash);
    ...
    public Result HandleGameCardAccessResult(Result result)
    {
        ...
        using UniqueLock<ReaderWriterLock> writeLock = new();
    ...
    public Result OpenOperator(ref SharedRef<IStorageDeviceOperator> outDeviceOperator)
    {
        ...
        using SharedRef<GameCardDeviceOperator> deviceOperator = new(new GameCardDeviceOperator(in storageDevice, _gc));
