...
public class GameCardManager : IStorageDeviceManager, IStorageDeviceOperator, IGameCardManager, IGameCardKeyManager
{
    ...
    {
        GameCardManager manager = new(gc, fsServer);

        using SharedRef<GameCardManager> sharedManager = new(manager);
    ...
    private Result LockAndHandleGameCardAccessResult(Result result)
    {
        ...
        using UniqueLock<ReaderWriterLock> writeLock = new(_rwLock);
    ...
    public Result IsHandleValid(out bool isValid, GameCardHandle handle)
    {
        ...
        using SharedLock<ReaderWriterLock> readLock = new();
    ...
    private Result OpenDeviceImpl(ref SharedRef<IStorageDevice> outStorageDevice, OpenGameCardAttribute attribute)
    {
        ...
        using SharedRef<IStorageDevice> storageDevice = new();
        using SharedRef<IStorage> baseStorage = new();
    ...
    {
        using UniqueRef<IStorage> storage = new(MakeReadOnlyGameCardStorage());
        if (!storage.HasValue)
    ...
    {
        using UniqueRef<IStorage> storage = new(MakeWriteOnlyGameCardStorage());
        if (!storage.HasValue)
            return ResultFs.AllocationMemoryFailedInGameCardManagerE.Log();
    ...
    {
        GameCardManagerOperationIdValue operation = (GameCardManagerOperationIdValue)operationId;
    ...
    private Result GetGameCardDeviceIdForProdCard(Span<byte> outBuffer, ReadOnlySpan<byte> devHeaderBuffer)
    {
        ...
        using PooledBuffer pooledBuffer = new(writeSize, writeSize);
    ...
    private Result EraseAndWriteParamDirectly(ReadOnlySpan<byte> inBuffer)
    {
        ...
        DevCardParameter devCardParam = SpanHelpers.AsReadOnlyStruct<DevCardParameter>(inBuffer);
    ...
    {
        using (SharedLock<ReaderWriterLock> readLock = new(_rwLock))
        {
            if (_state == CardState.Initial || _gc.IsCardActivationValid())
            {
