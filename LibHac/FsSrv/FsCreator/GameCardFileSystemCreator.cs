    ...
    public Result Create(ref SharedRef<IFileSystem> outFileSystem, GameCardHandle handle, GameCardPartition partitionType)
    {
        ...
        using (ScopedLock.Lock(ref _mutex))
        {
            ...
            if (!_rootPartition.HasValue || !_rootPartition.Get.IsValid())
            {
                ...
                using SharedRef<IStorage> rootStorage = new();
                ...
                const int dataAlignment = 512;
                using SharedRef<IStorage> alignedRootStorage = new(
                ...
                using SharedRef<IStorage> rootFsStorage = new(new SubStorage(in alignedRootStorage,
                ...
                using UniqueRef<Sha256PartitionFileSystemMeta> rootPartitionFsMeta = new(new Sha256PartitionFileSystemMeta());
        ...
        using SharedRef<IStorage> partitionStorage = new();
        ...
        using UniqueRef<Sha256PartitionFileSystemMeta> partitionFsMeta = new(new Sha256PartitionFileSystemMeta());
        ...
        using SharedRef<Sha256PartitionFileSystem> fs = new(new Sha256PartitionFileSystem());
        if (!fs.HasValue)
