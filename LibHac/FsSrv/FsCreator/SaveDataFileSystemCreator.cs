...
file static class Anonymous
{
        ...
        IHash256GeneratorFactorySelector hashGeneratorFactorySelector, uint minimumVersion)
    {
        ...
        using ValueSubStorage saveSubStorage = new(saveImageStorage, 0, storageSize);
...
public class SaveDataFileSystemCreator : ISaveDataFileSystemCreator
{
        ...
        ISaveDataCommitTimeStampGetter timeStampGetter, bool isReconstructible)
    {
        ...
        if (type == DirectoryEntryType.Directory)
        {
            ...
            using UniqueRef<SubdirectoryFileSystem> baseFs = new(new SubdirectoryFileSystem(in baseFileSystem));
            ...
            using SharedRef<DirectorySaveDataFileSystem> saveDirFs = new(
        ...
        {
            using SharedRef<IStorage> fileStorage = new();
            ...
            {
                using SharedRef<ApplicationTemporaryFileSystem> fs = new(new ApplicationTemporaryFileSystem());
            ...
            {
                using SharedRef<SaveDataFileSystem> fs = new(new SaveDataFileSystem());
                if (!fs.HasValue)
        ...
        using SharedRef<SaveDataResultConvertFileSystem> resultConvertFs = new(
        ...
        bool isReconstructible)
    {
        ...
        using SharedRef<SaveDataFileSystem> saveFs = new(new SaveDataFileSystem());
        ...
        using SharedRef<SaveDataInternalStorageFileSystem> internalStorage = new(new SaveDataInternalStorageFileSystem());
        ...
        IBufferManager bufferManager, bool isDeviceUniqueMac, bool isReconstructible)
    {
        ...
        using SharedRef<FileStorageBasedFileSystem> fileStorage = new();
        ...
        using ValueSubStorage fileSubStorage = new(fileStorage.Get, 0, size);
