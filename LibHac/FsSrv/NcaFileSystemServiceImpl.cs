...
file static class Anonymous
{
    ...
    public static Result LoadNspdVerificationData(ref CodeVerificationData outCodeVerificationData, IFileSystem fileSystem)
    {
        ...
        using UniqueRef<IFile> verificationDataFile = new();

        using Path pathVerificationData = new();
...
public class NcaFileSystemServiceImpl : IDisposable
{
        ...
        ulong id, bool isDirectory)
    {
        ...
        U8Span currentPath = new(path.GetString());
        ...
        if (isDirectory)
        {
            ...
            if (type == FileSystemProxyType.Manual)
            {
                ...
                using SharedRef<IFileSystem> readOnlyFileSystem = new(new ReadOnlyFileSystem(in hostFileSystem));
                outFileSystem.SetByMove(ref readOnlyFileSystem.Ref);

                return Result.Success;
            }

            using SharedRef<IFileSystem> ncdFileSystem = new();
            res = _config.SubDirectoryFsCreator.Create(ref ncdFileSystem.Ref, in baseFileSystem, in directoryPath);
            if (res.IsFailure()) return res.Miss();

            using SharedRef<IFileSystem> fsPartitionOnNcdFileSystem = new();
            ...
            if (type == FileSystemProxyType.Code)
            {
                ...
                using SharedRef<NpdmVerificationFileSystem> codeFileSystem =
                    new(new NpdmVerificationFileSystem(in fsPartitionOnNcdFileSystem, hash));
        ...
        using SharedRef<NcaReader> ncaReader = new();
        ...
        ContentAttributes attributes, FileSystemProxyType type, ulong programId, bool isDirectory)
    {
        ...
        if (isDirectory)
        {
            ...
            using SharedRef<ReadOnlyFileSystem> readOnlyFileSystem = new(new ReadOnlyFileSystem(in hostFileSystem));
        ...
        bool canMountSystemDataPrivate)
    {
        ...
        using SharedRef<NcaReader> originalNcaReader = new();
        ...
        using SharedRef<NcaReader> currentNcaReader = new();
        ...
        ContentStorageId contentStorageId)
    {
        ...
        {
            U8StringBuilder sb = new(contentStoragePathBuffer);
        ...
        using scoped Path contentStoragePath = new();
        ...
        using SharedRef<IFileSystem> subDirFs = new();
        ...
        ref readonly SharedRef<IFileSystem> baseFileSystem, FileSystemProxyType type)
    {
        ...
        using scoped Path directoryPath = new();
        ...
        bool isGameCard, bool canMountSystemDataPrivate)
    {
        ...
        NcaFsHeaderReader ncaFsHeaderReader = new();
