...
public readonly struct BaseFileSystemService
{
        ...
        BisPartitionId partitionId)
    {
        ...
        using SharedRef<IFileSystem> subDirFileSystem = new();
        res = Utility.CreateSubDirectoryFileSystem(ref subDirFileSystem.Ref, in fileSystem, in pathNormalized);
        if (res.IsFailure()) return res.Miss();

        // Add all the file system wrappers
        using SharedRef<IFileSystem> typeSetFileSystem = new(new StorageLayoutTypeSetFileSystem(in subDirFileSystem, storageFlag));
        ...
        ulong transferMemorySize)
    {
        ...
        using UniqueRef<IWiper> bisWiper = new();
