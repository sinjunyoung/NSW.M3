...
internal class NcaFileSystemService : IRomFileSystemAccessFailureManager
{
    ...
    {
        // Create the service
        NcaFileSystemService ncaService = new(serviceImpl, processId);

        // Wrap the service in a ref-counter and give the service a weak self-reference
        using SharedRef<NcaFileSystemService> sharedService = new(ncaService);
    ...
    {
        ProgramRegistryImpl programRegistry = new(_serviceImpl.FsServer);
        ...
        StorageId storageId)
    {
        ...
        using SharedRef<IAsynchronousAccessSplitter> storageAccessSplitter = new();
    ...
    public Result OpenDataStorageByCurrentProcess(ref SharedRef<IStorageSf> outStorage)
    {
        ...
        using UniqueRef<IUniqueLock> romMountCountSemaphore = new();
        ...
        using UniqueRef<IUniqueLock> romDivisionSizeUnitCountSemaphore = new();
        ...
        using SharedRef<IStorage> retryStorage = new(new DeepRetryStorage(in storage, in storageAccessSplitter,
        ...
        using SharedRef<IStorage> typeSetStorage = new(new StorageLayoutTypeSetStorage(in retryStorage, storageFlag));
        using SharedRef<IStorageSf> storageAdapter = new(new StorageInterfaceAdapter(in typeSetStorage));
    ...
    public Result OpenDataStorageByDataId(ref SharedRef<IStorageSf> outStorage, DataId dataId, StorageId storageId)
    {
        ...
        using Path systemDataPath = new();
        using Path systemDataPatchPath = new();
        ...
        Accessibility accessibility = programInfo.AccessControl.GetAccessibilityFor(AccessibilityType.MountSystemDataPrivate);
    ...
    public Result OpenDataFileSystemByDataId(ref SharedRef<IFileSystemSf> outFileSystem, DataId dataId, StorageId storageId)
    {
        ...
        using Path dataPath = new();
        using Path dataPatchPathUnused = new();
    ...
    public Result GetRightsId(out RightsId outRightsId, ProgramId programId, StorageId storageId)
    {
        ...
        using ScopedStorageLayoutTypeSetter scopedContext = new(StorageLayoutType.All);
        ...
        ContentStorageId contentStorageId)
    {
        ...
        // Add all the file system wrappers
        using SharedRef<IFileSystem> typeSetFileSystem = new(new StorageLayoutTypeSetFileSystem(in fileSystem, storageFlag));
        using SharedRef<IFileSystem> alignmentMatchableFileSystem = new(new AlignmentMatchableFileSystem(in typeSetFileSystem));
        using SharedRef<IFileSystem> asyncFileSystem = new(new AsynchronousAccessFileSystem(in alignmentMatchableFileSystem));
        using SharedRef<IFileSystemSf> fileSystemAdapter = FileSystemInterfaceAdapter.CreateShared(in asyncFileSystem, allowAllOperations: false);
    ...
    public Result OpenSystemDataUpdateEventNotifier(ref SharedRef<IEventNotifier> outEventNotifier)
    {
        ...
        using UniqueRef<SystemDataUpdateEventNotifier> systemDataUpdateEventNotifier = new();
