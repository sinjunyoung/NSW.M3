...
internal static class MmcService
{
        ...
        MmcPartition partition)
    {
        ...
        using SharedRef<IStorageSf> mmcStorage = new();
        res = storageDeviceManager.Get.OpenStorage(ref mmcStorage.Ref, attribute);
        if (res.IsFailure()) return res.Miss();

        using SharedRef<IStorage> storage = new(new StorageServiceObjectAdapter(in mmcStorage));

        if (IsSpeedEmulationNeeded(partition))
        {
            using SharedRef<IStorage> emulationStorage = new(new SpeedEmulationStorage(in storage, service.FsSrv));
    ...
    public static Result GetMmcSpeedMode(this StorageService service, out MmcSpeedMode speedMode)
    {
        ...
        using SharedRef<IStorageDeviceOperator> mmcOperator = new();
