...
internal class SaveDataFileSystemService : ISaveDataTransferCoreInterface, ISaveDataMultiCommitCoreInterface
{
    ...
    {
        // Create the service
        SaveDataFileSystemService saveService = new(serviceImpl, processId);

        // Wrap the service in a ref-counter and give the service a weak self-reference
        using SharedRef<SaveDataFileSystemService> sharedService = new(saveService);
    ...
    {
        ProgramRegistryImpl registry = new(_serviceImpl.FsServer);
    ...
    public Result GetFreeSpaceSizeForSaveData(out long outFreeSpaceSize, SaveDataSpaceId spaceId)
    {
        ...
        using ScopedStorageLayoutTypeSetter scopedContext = new(StorageLayoutType.NonGameCard);
    ...
    {
        using ScopedStorageLayoutTypeSetter scopedContext = new(StorageLayoutType.Bis);
        ...
        using UniqueRef<SaveDataIndexerAccessor> accessor = new();
    ...
    public Result SwapSaveDataKeyAndState(SaveDataSpaceId spaceId, ulong saveDataId1, ulong saveDataId2)
    {
        ...
        SaveDataState rhsState = rhsValue.State;
    ...
    public Result SetSaveDataRootPath(ref readonly FspPath path)
    {
        ...
        using Path saveDataRootPath = new();
    ...
    public Result OpenSaveDataFile(ref SharedRef<IFile> outFile, SaveDataSpaceId spaceId, ulong saveDataId,
        OpenMode openMode)
    {
        StorageLayoutType storageFlag = StorageLayoutType.NonGameCard;
        using ScopedStorageLayoutTypeSetter scopedContext = new(storageFlag);

        using SharedRef<IFile> file = new();
        Result res = _serviceImpl.OpenSaveDataFile(ref file.Ref, spaceId, saveDataId, openMode);
        if (res.IsFailure()) return res.Miss();

        using SharedRef<IFile> typeSetFile = new(new StorageLayoutTypeSetFile(ref file.Ref, storageFlag));
    ...
    public Result ExtendSaveDataFileSystem(SaveDataSpaceId spaceId, ulong saveDataId, long dataSize, long journalSize)
    {
        ...
        using (UniqueRef<SaveDataIndexerAccessor> accessor = new())
        {
            ...
            if (result.IsFailure())
            {
                ...
                U8StringBuilder sb = new(stringBuffer, true);
        ...
        in SaveDataAttribute attribute, ProgramInfo programInfo, bool openReadOnly)
    {
        ...
        using UniqueRef<IUniqueLock> mountCountSemaphore = new();
        ...
        using SharedRef<IFileSystem> typeSetFileSystem = new(new StorageLayoutTypeSetFileSystem(in fileSystem, storageFlag));
        using SharedRef<IFileSystem> asyncFileSystem = new();
        ...
        using SharedRef<IEntryOpenCountSemaphoreManager> openEntryCountAdapter =
            new(new SaveDataOpenCountAdapter(in saveService));

        using SharedRef<IFileSystem> openCountFileSystem = new(new OpenCountFileSystem(in asyncFileSystem,
        ...
        SaveDataSpaceId spaceId, in SaveDataAttribute attribute)
    {
        ...
        using SharedRef<IFileSystem> openCountFileSystem = new(
    ...
    public Result OpenSaveDataInfoReader(ref SharedRef<ISaveDataInfoReader> outInfoReader)
    {
        ...
        using SharedRef<SaveDataInfoReaderImpl> reader = new();
        ...
        SaveDataSpaceId spaceId)
    {
        ...
        using UniqueRef<SaveDataInfoFilterReader> filterReader = new();
        ...
        using (UniqueRef<SaveDataIndexerAccessor> accessor = new())
        {
            ...
            SaveDataInfoFilter filter = new(ConvertToRealSpaceId(spaceId), programId: default,
        ...
        SaveDataSpaceId spaceId, in SaveDataFilter filter)
    {
        ...
        using (UniqueRef<SaveDataIndexerAccessor> accessor = new())
        {
            ...
            SaveDataInfoFilter infoFilter = new(ConvertToRealSpaceId(spaceId), in filter);
        ...
        in SaveDataInfoFilter infoFilter)
    {
        ...
        using UniqueRef<SaveDataInfoFilterReader> filterReader =
            new(new SaveDataInfoFilterReader(in reader, in infoFilter));
        ...
        in SaveDataFilter filter)
    {
        ...
        SaveDataInfoFilter infoFilter = new(ConvertToRealSpaceId(spaceId), in tempFilter);

        res = FindSaveDataWithFilterImpl(out long outCount, out saveDataInfoBuffer.As<SaveDataInfo>(), spaceId, in infoFilter);
        if (res.IsFailure()) return res.Miss();
        ...
        SaveDataSpaceId spaceId, ulong saveDataId)
    {
        ...
        // Add all the wrappers for the file system
        using SharedRef<IFileSystem> asyncFileSystem = new(new AsynchronousAccessFileSystem(in fileSystem));
    ...
    public Result QuerySaveDataInternalStorageTotalSize(out long outTotalSize, SaveDataSpaceId spaceId, ulong saveDataId)
    {
        ...
        {
            using InternalStorageFilePath path = new();
        ...
        SaveDataSpaceId spaceId)
    {
        ...
        using (UniqueRef<SaveDataIndexerAccessor> accessor = new())
        {
            ...
            SaveDataInfoFilter filter = new(ConvertToRealSpaceId(spaceId), resolvedProgramId, SaveDataType.Cache,
        ...
        SaveDataMetaType metaType, OpenMode mode)
    {
        ...
        using scoped Path pathMeta = new();
        ...
        in SaveDataAttribute attribute, SaveDataMetaType metaType)
    {
        ...
        using SharedRef<IFileSystem> tmpFileSystem = new();
        res = _serviceImpl.OpenSaveDataMetaDirectoryFileSystem(ref tmpFileSystem.Ref, spaceId, targetSaveDataId);
        if (res.IsFailure()) return res.Miss();

        using SharedRef<IFileSystem> typeSetFileSystem = new(new StorageLayoutTypeSetFileSystem(in tmpFileSystem, storageFlag));
        using SharedRef<IFileSystem> asyncFileSystem = new(new AsynchronousAccessFileSystem(in typeSetFileSystem));
        ...
        U8StringBuilder sb = new(SpanHelpers.AsByteSpan(ref sfPath));
        sb.Append((byte)'/').AppendFormat((uint)metaType, 'x', 8).Append(".meta"u8);

        using SharedRef<ISaveDataTransferProhibiter> prohibiter = new();
        ...
        using SharedRef<IFileSf> file = new();
    ...
    private Result GetCacheStorageSpaceId(out SaveDataSpaceId spaceId, ulong programId)
    {
        ...
        Result DoesCacheStorageExist(out bool exists, SaveDataSpaceId saveSpaceId)
        {
            ...
            SaveDataInfoFilter filter = new(ConvertToRealSpaceId(saveSpaceId), new ProgramId(programId),
    ...
    public Result OpenSaveDataTransferManager(ref SharedRef<ISaveDataTransferManager> outManager)
    {
        ...
        using SharedRef<ISaveDataTransferManager> manager = new(
    ...
    public Result OpenSaveDataTransferManagerVersion2(ref SharedRef<ISaveDataTransferManagerWithDivision> outManager)
    {
        ...
        using SharedRef<ISaveDataTransferCoreInterface> transferInterface = GetSaveDataTransferCoreInterfaceFromThis();
        using SharedRef<ISaveDataTransferManagerWithDivision> manager = new(
        ...
        ref SharedRef<ISaveDataTransferManagerForSaveDataRepair> outManager)
    {
        ...
        using SharedRef<ISaveDataTransferCoreInterface> transferInterface = GetSaveDataTransferCoreInterfaceFromThis();
        using SharedRef<ISaveDataTransferManagerForSaveDataRepair> manager = new(
    ...
    public Result OpenSaveDataTransferManagerForRepair(ref SharedRef<ISaveDataTransferManagerForRepair> outManager)
    {
        ...
        using SharedRef<ISaveDataTransferCoreInterface> transferInterface = GetSaveDataTransferCoreInterfaceFromThis();
        using SharedRef<ISaveDataTransferManagerForRepair> manager = new(
        ...
        Ncm.ApplicationId applicationId)
    {
        ...
        using SharedRef<SaveDataPorterProhibiter> prohibiter = new(
        ...
        SaveDataSpaceId destinationSpaceId, NativeHandle workBufferHandle, ulong workBufferSize)
    {
        ...
        using SharedRef<ISaveDataTransferCoreInterface> transferInterface = GetSaveDataTransferCoreInterfaceFromThis();
        using SharedRef<ISaveDataMover> mover = new(new SaveDataMover(in transferInterface, sourceSpaceId,
        ...
        int startIndex, int bufferIdCount)
    {
        ...
        Span<Ncm.ApplicationId> ids = Span<Ncm.ApplicationId>.Empty;
    ...
    public Result OverrideSaveDataTransferTokenSignVerificationKey(InBuffer key)
    {
        ...
        SaveDataTransferCryptoConfiguration cryptoConfig = _serviceImpl.GetSaveDataTransferCryptoConfiguration();
