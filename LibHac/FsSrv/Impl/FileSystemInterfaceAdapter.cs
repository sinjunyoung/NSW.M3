...
public class FileInterfaceAdapter : IFileSf
{
    ...
    public Result Write(long offset, InBuffer source, long size, WriteOption option)
    {
        ...
        using ScopedThreadPriorityChangerByAccessPriority scopedPriorityChanger =
            new(ScopedThreadPriorityChangerByAccessPriority.AccessMode.Write);
...
public class FileSystemInterfaceAdapter : IFileSystemSf
{
    ...
    {
        FileSystemInterfaceAdapter adapter = new(in baseFileSystem, allowAllOperations);
        using SharedRef<FileSystemInterfaceAdapter> sharedAdapter = new(adapter);
    ...
    {
        FileSystemInterfaceAdapter adapter = new(in baseFileSystem, flags, allowAllOperations);
        using SharedRef<FileSystemInterfaceAdapter> sharedAdapter = new(adapter);
    ...
    public static PathFlags GetDefaultPathFlags() => new();
    ...
    {
        using Path currentPathNormalized = new();
        Result res = SetUpPath(ref currentPathNormalized.Ref(), in currentPath);
        if (res.IsFailure()) return res.Miss();

        using Path newPathNormalized = new();
    ...
    public Result OpenFile(ref SharedRef<IFileSf> outFile, ref readonly PathSf path, uint mode)
    {
        ...
        FileInterfaceAdapter adapter = new(ref file.Ref, in selfReference, _allowAllOperations);
    ...
    public Result OpenDirectory(ref SharedRef<IDirectorySf> outDirectory, ref readonly PathSf path, uint mode)
    {
        ...
        DirectoryInterfaceAdapter adapter = new(ref directory.Ref, in selfReference);
