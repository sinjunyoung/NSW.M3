...
public static class ProgramInfoGlobalMethods
{
    public static bool IsInitialProgram(this FileSystemServer fsSrv, ulong processId)
    {
        ...
        using (InitializationGuard guard = new(ref g.InitialProcessIdRangeInitGuard, fsSrv.Globals.InitMutex))
        {
    ...
    {
        ref ProgramInfoGlobals g = ref fsSrv.Globals.ProgramInfo;
        using (InitializationGuard guard = new(ref g.CurrentProcessIdInitGuard, fsSrv.Globals.InitMutex))
        {
            if (!guard.IsInitialized)
            {
...
public class ProgramInfo
{
    ...
    {
        ref ProgramInfoGlobals g = ref fsSrv.Globals.ProgramInfo;
        using (InitializationGuard guard = new(ref g.ProgramInfoForInitialProcessInitGuard, fsSrv.Globals.InitMutex))
        {
            if (!guard.IsInitialized)
            {
