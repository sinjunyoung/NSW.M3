    ...
    public static Result WipeData(IFileSystem fileSystem, ref readonly Path filePath, RandomDataGenerator generateKey)
    {
        ...
        using (UniqueRef<IFile> file = new())
        {
            ...
            try
            {
                ...
                using PooledBuffer workBuffer = new();
...
public class SaveDataFileSystemServiceImpl : IDisposable
{
    ...
    public Result DoesSaveDataEntityExist(out bool exists, SaveDataSpaceId spaceId, ulong saveDataId)
    {
        ...
        using scoped Path saveImageName = new();
        ...
        OpenMode openMode)
    {
        ...
        using SharedRef<IFile> saveDataFile = new();
        ...
        ulong saveDataId, ref readonly Path saveDataRootPath, bool openReadOnly, SaveDataType type, bool cacheExtraData)
    {
        ...
        if (isEmulatedOnHost)
        {
            ...
            using scoped Path saveDirectoryName = new();
        ...
        using SharedRef<ISaveDataFileSystem> saveDataFs = new();
        ...
        using SharedRef<SaveDataFileSystemCacheRegister> registerFs = new(
        ...
        if (openReadOnly)
        {
            ...
            using SharedRef<ReadOnlyFileSystem> readOnlyFileSystem = new(new ReadOnlyFileSystem(in tempFs));
        ...
        SaveDataSpaceId spaceId, ulong saveDataId)
    {
        ...
        using scoped Path saveDataMetaIdDirectoryName = new();
        ...
        bool isReconstructible)
    {
        ...
        using SharedRef<IFileSystem> internalStorage = new();
        ...
        SaveDataSpaceId spaceId, long dataSize, long journalSize, bool createIfMissing, bool isReconstructible)
    {
        ...
        if (result.IsFailure())
        {
            if (ResultFs.PathNotFound.Includes(result))
            {
                ...
                using SharedRef<IStorage> saveDataStorage = new(new FileStorage(saveDataFile));

                // Check that the new save data sizes aren't smaller than the current sizes
                using SharedRef<ISaveDataExtraDataAccessor> extraDataAccessor = new();
                ...
                using FileStorage contextStorage = new(outFile.Get);
        ...
        SaveDataType type, long dataSize, long journalSize, ref readonly Path saveDataRootPath, bool isExtensionStart)
    {
        ...
        using UniqueRef<IFile> saveDataFile = new();
        using UniqueRef<IFile> contextFile = new();
        ...
        using SharedRef<IStorage> saveDataStorage = new(new FileStorage(saveDataFile.Get));
        FileStorage contextStorage = new(contextFile.Get);
        ...
        using ValueSubStorage saveDataSubStorage = new(saveDataStorage.Get, 0, extendedTotalSize);
        using ValueSubStorage logSubStorage = new(contextStorage, SaveDataExtender.QueryContextSize(), extender.GetLogSize());
        ...
        long metaFileSize)
    {
        ...
        using scoped Path saveDataMetaName = new();
    ...
    public Result DeleteAllSaveDataMetas(ulong saveDataId, SaveDataSpaceId spaceId)
    {
        ...
        using scoped Path saveDataMetaDirectoryName = new();
        ...
        using scoped Path saveDataIdDirectoryName = new();
        ...
        ref readonly Path saveDataRootPath, bool skipFormat)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                Optional<IntegritySaveDataParameters> integrityParams = new();
                Optional<JournalIntegritySaveDataParameters> journalIntegrityParams = new();
                ...
                using SharedRef<FileStorageBasedFileSystem> fileStorage = new(new FileStorageBasedFileSystem());
                ...
                if (!isJournalingSupported)
                {
                    ...
                    using ValueSubStorage subStorage = new(fileStorage.Get, 0, totalSize);
                ...
                {
                    Optional<HashSalt> hashSaltRandom = new();
        ...
        ulong saveDataId, SaveDataType type, ref readonly Path saveDataRootPath)
    {
        ...
        using SharedRef<IFileSystem> unusedSaveDataFs = new();
        using SharedRef<ISaveDataExtraDataAccessor> extraDataAccessor = new();
        ...
        SaveDataSpaceId spaceId, ulong saveDataId, ref readonly Path saveDataRootPath, bool allowEmulatedSave)
    {
        ...
        if (allowEmulatedSave && IsAllowedDirectorySaveData(spaceId, in saveDataRootPath))
        {
            if (UseTargetManager)
            {
                using (SharedRef<IFileSystem> tmFileSystem = new())
                {
        ...
        using scoped Path saveDataAreaDirectoryName = new();
    ...
    {
        using SharedRef<IFileSystem> baseFileSystem = new();
        ...
        switch (spaceId)
        {
            ...
            case SaveDataSpaceId.SdUser:
            {
                ...
                using scoped Path pathParent = new();
                ...
                using scoped Path pathSdRoot = new();
