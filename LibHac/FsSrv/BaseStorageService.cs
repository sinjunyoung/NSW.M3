...
public readonly struct BaseStorageService
{
    ...
    {
        StorageLayoutType storageFlag = StorageLayoutType.Bis;
        using ScopedStorageLayoutTypeSetter scopedLayoutType = new(storageFlag);
        ...
        using SharedRef<IStorage> typeSetStorage = new(new StorageLayoutTypeSetStorage(in storage, storageFlag));
        // Todo: Async storage
        ...
        GameCardPartitionRaw partitionId)
    {
        ...
        // Todo: Async storage
        using SharedRef<IStorageSf> storageAdapter = new(new StorageInterfaceAdapter(in storage));
    ...
    public Result OpenDeviceOperator(ref SharedRef<IDeviceOperator> outDeviceOperator)
    {
        ...
        using SharedRef<IDeviceOperator> deviceOperator =
            new(new DeviceOperator(_serviceImpl.FsServer, programInfo.AccessControl,
                _processId));
...
public class BaseStorageServiceImpl
{
    ...
    {
        ProgramRegistryImpl registry = new(_config.FsServer);
