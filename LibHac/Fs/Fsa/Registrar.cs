...
public static class Registrar
{
    ...
    {
        using UniqueRef<ISaveDataAttributeGetter> attributeGetter = new();
        using UniqueRef<ICommonMountNameGenerator> mountNameGenerator = new();

        using UniqueRef<FileSystemAccessor> accessor = new(new FileSystemAccessor(fs.Hos, name, null,
    ...
    {
        using UniqueRef<IUnmountHookInvoker> unmountHookInvoker = new();
        using UniqueRef<ISaveDataAttributeGetter> attributeGetter = new();
        ...
        IStorage storageForPurgeFileDataCache, bool usePathCache, ref UniqueRef<IUnmountHookInvoker> unmountHook)
    {
        ...
        using (UniqueRef<UnmountHookFileSystem> unmountHookFileSystem =
            new(new UnmountHookFileSystem(ref fileSystem, ref unmountHook)))
        {
        ...
        using UniqueRef<FileSystemAccessor> accessor = new(new FileSystemAccessor(fs.Hos, name,
