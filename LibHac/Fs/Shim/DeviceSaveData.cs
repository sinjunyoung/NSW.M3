...
public static class DeviceSaveData
{
        ...
        in SaveDataAttribute attribute)
    {
        ...
        FileSystemServiceObjectAdapter fileSystemAdapterRaw = new(in fileSystem);
        using UniqueRef<IFileSystem> fileSystemAdapter = new(fileSystemAdapterRaw);

        if (!fileSystemAdapter.HasValue)
            return ResultFs.AllocationMemoryFailedInDeviceSaveDataA.Log();

        using UniqueRef<ISaveDataAttributeGetter> saveDataAttributeGetter =
            new(new DeviceSaveDataAttributeGetter(attribute.ProgramId));
    ...
    public static bool IsDeviceSaveDataExisting(this FileSystemClient fs, ApplicationId applicationId)
    {
        ...
        bool exists;
        Ncm.ApplicationId appId = new(applicationId.Value);
