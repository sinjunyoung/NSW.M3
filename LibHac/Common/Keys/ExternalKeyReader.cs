...
public static class ExternalKeyReader
{
        ...
        string consoleKeysFilename = null, IProgressReport logger = null)
    {
        ...
        if (filename != null)
        {
            using FileStream storage = new(filename, FileMode.Open, FileAccess.Read);
            ReadMainKeys(keySet, storage, keyInfos, logger);
        }

        if (consoleKeysFilename != null)
        {
            using FileStream storage = new(consoleKeysFilename, FileMode.Open, FileAccess.Read);
            ReadMainKeys(keySet, storage, keyInfos, logger);
        }

        if (titleKeysFilename != null)
        {
            using FileStream storage = new(titleKeysFilename, FileMode.Open, FileAccess.Read);
        ...
        string titleKeysFilename = null, string consoleKeysFilename = null, IProgressReport logger = null)
    {
        ...
        if (prodKeysFilename != null)
        {
            ...
            using FileStream storage = new(prodKeysFilename, FileMode.Open, FileAccess.Read);
            ReadMainKeys(keySet, storage, keyInfos, logger);
        }
        ...
        if (devKeysFilename != null)
        {
            ...
            using FileStream storage = new(devKeysFilename, FileMode.Open, FileAccess.Read);
            ReadMainKeys(keySet, storage, keyInfos, logger);
        }
    ...
    public static void ReadMainKeys(KeySet keySet, Stream reader, List<KeyInfo> keyList, IProgressReport logger = null)
    {
        ...
        KvPairReaderContext ctx = new(streamReader, buffer);
    ...
    public static void ReadTitleKeys(KeySet keySet, Stream reader, IProgressReport logger = null)
    {
        ...
        while (true)
        {
            ...
            else if (status == ReaderStatus.ReadKey)
            {
                ...
                RightsId rightsId = new();
                AccessKey titleKey = new();
