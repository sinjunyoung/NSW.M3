...
{
    private Lazy<Dictionary<Result, string>> ResultNames { get; } = new(GetResultNames);
    ...
    {
        ResultArchiveReader archiveReader = new(DecompressArchive());
        return archiveReader.GetDictionary();
    }

    private static byte[] DecompressArchive()
    {
        using DeflateStream deflateStream = new(new MemoryStream(ArchiveData.ToArray()), CompressionMode.Decompress);
        using MemoryStream archiveDataStream = new();
    ...
    private readonly ref struct ResultArchiveReader
    {
        ...
        {
            Dictionary<Result, string> dict = new();
            ...
            foreach (ref readonly Element element in elements)
            {
                ...
                Result result = new(element.Module, element.DescriptionStart);
