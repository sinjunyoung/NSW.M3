...
public class HorizonResultException : LibHacException
{
    ...
    /// <summary>
    ///     Gets if this <see cref="HorizonResultException"/> is of the specified <see cref="Result.Base"/>.
    /// </summary>
    public bool IsOfResultType(Result.Base resultType) =>
        ResultValue == resultType.Value || InternalResultValue == resultType.Value;

    /// <summary>
    ///     Gets if this <see cref="HorizonResultException"/>'s internal result matches the string error code.
    /// </summary>
    public bool IsOfResultCode(string errorCode) =>
        ResultValue.ErrorCode.Equals(errorCode) || InternalResultValue.ErrorCode.Equals(errorCode);
}