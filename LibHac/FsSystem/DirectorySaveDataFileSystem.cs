...
public class DirectorySaveDataFileSystem : ISaveDataFileSystem
{
    ...
    {
        using Path pathDirectoryName = new();
    ...
    {
        using Path currentFullPath = new();
        using Path newFullPath = new();
    ...
    protected override Result DoOpenFile(ref UniqueRef<IFile> outFile, ref readonly Path path, OpenMode mode)
    {
        ...
        using UniqueRef<IFile> baseFile = new();
        res = _baseFs.OpenFile(ref baseFile.Ref, in fullPath, mode);
        if (res.IsFailure()) return res.Miss();

        using UniqueRef<IFile> file = new(new DirectorySaveDataFile(ref baseFile.Ref, this, mode));
    ...
    private Result SynchronizeDirectory(ref readonly Path destPath, ref readonly Path sourcePath)
    {
        ...
        if (_fsClient is not null)
        {
            ...
            using (RentedArray<byte> buffer = new(IdealWorkBufferSize))
            {
    ...
    private Result DoCommit(bool updateTimeStamp)
    {
        ...
        using RetryClosure closure = new(this);
    ...
    private Result SynchronizeExtraData(ref readonly Path destPath, ref readonly Path sourcePath)
    {
        ...
        using (UniqueRef<IFile> sourceFile = new())
        {
        ...
        using (UniqueRef<IFile> destFile = new())
        {
