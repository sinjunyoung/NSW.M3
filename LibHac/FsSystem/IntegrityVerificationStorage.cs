...
public class IntegrityVerificationStorage : IStorage
{
    ...
    public override Result Read(long offset, Span<byte> destination)
    {
        ...
        using UniqueRef<IHash256Generator> hashGenerator = new();
        res = _hashGeneratorFactory.Create(ref hashGenerator.Ref);
        if (res.IsFailure()) return res.Miss();

        int signatureCount = destination.Length >> _verificationBlockOrder;
        using PooledBuffer signatureBuffer =
            new(signatureCount * Unsafe.SizeOf<BlockHash>(), Unsafe.SizeOf<BlockHash>());
        ...
        ReadOnlySpan<byte> inBuffer)
    {
        ...
        switch (operationId)
        {
            case OperationId.FillZero:
            {
                ...
                using RentedArray<byte> workBuffer = new(bufferSize);
            ...
            case OperationId.DestroySignature:
            {
                ...
                using RentedArray<byte> workBuffer = new((int)signSize);
                if (workBuffer.Array is null)
