...
internal static class Utility
{
        ...
        Span<byte> workBuffer)
    {
        ...
        FsIterationTaskClosure closure = new();
    ...
    public static Result VerifyDirectoryRecursively(IFileSystem fileSystem, Span<byte> workBuffer)
    {
        ...
        DirectoryEntry dirEntryBuffer = new();
    ...
    {
        using Path pathCopy = new();
        bool isFinished;

        Result res = pathCopy.Initialize(in path);
        if (res.IsFailure()) return res.Miss();

        using DirectoryPathParser parser = new();
    ...
    {
        using UniqueLock<SemaphoreAdapter> uniqueLock = new(semaphore, new DeferLock());
    ...
    {
        using UniqueLock<SemaphoreAdapter> semaphoreAdapter = new();
        Result res = TryAcquireCountSemaphore(ref semaphoreAdapter.Ref(), semaphore);
        if (res.IsFailure()) return res.Miss();

        UniqueLockWithPin<T> lockWithPin = new(ref semaphoreAdapter.Ref(), in objectToPin);
        using UniqueRef<IUniqueLock> uniqueLock = new(lockWithPin);
    ...
    {
        using UniqueRef<MultilockWithPin<T>> multilock = new(new(in objectToPin, semaphore));
    ...
    {
        using UniqueRef<StageLockWithPin<T>> stageLock = new(new(ref inLock, in objectToPin, semaphore));
