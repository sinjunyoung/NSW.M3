...
public class ConcatenationFileSystem : IFileSystem
{
    private class ConcatenationFile : IFile
    {
        ...
        protected override Result DoWrite(long offset, ReadOnlySpan<byte> source, in WriteOption option)
        {
            ...
            WriteOption internalFileOption = new(option.Flags & ~WriteOptionFlag.Flush);
        ...
        protected override Result DoSetSize(long size)
        {
            ...
            using Path internalFilePath = new();
            ...
            if (size > currentSize)
            {
                ...
                for (int i = currentTailIndex + 1; i <= newTailIndex; i++)
                {
                    ...
                    using UniqueRef<IFile> newInternalFile = new();
            ...
            ReadOnlySpan<byte> inBuffer)
        {
            switch (operationId)
            {
                ...
                case OperationId.QueryRange:
                {
                    ...
                    OperateRangeClosure closure = new();
    ...
    {
        using ScopedLock<SdkMutexType> scopedLock = new(ref _mutex);
    ...
    protected override Result DoOpenFile(ref UniqueRef<IFile> outFile, ref readonly Path path, OpenMode mode)
    {
        ...
        List<IFile> internalFiles = new(fileCount);

        using Path filePath = new();
        ...
        try
        {
            for (int i = 0; i < fileCount; i++)
            {
                ...
                using UniqueRef<IFile> internalFile = new();
            ...
            using UniqueRef<ConcatenationFile> concatFile = new(
        ...
        OpenDirectoryMode mode)
    {
        ...
        using UniqueRef<IDirectory> baseDirectory = new();
        Result res = _baseFileSystem.Get.OpenDirectory(ref baseDirectory.Ref, in path, OpenDirectoryMode.All);
        if (res.IsFailure()) return res.Miss();

        using UniqueRef<ConcatenationDirectory> concatDirectory = new(
