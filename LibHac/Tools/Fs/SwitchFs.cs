...
public class SwitchFs : IDisposable
{
    ...
    public Dictionary<string, SwitchFsNca> Ncas { get; } = new(StringComparer.OrdinalIgnoreCase);
    public Dictionary<string, SaveDataFileSystem> Saves { get; } = new(StringComparer.OrdinalIgnoreCase);
    public Dictionary<ulong, Title> Titles { get; } = new();
    public Dictionary<ulong, Application> Applications { get; } = new();
    ...
    public static SwitchFs OpenSdCard(KeySet keySet, ref UniqueRef<IAttributeFileSystem> fileSystem)
    {
        ...
        {
            SubdirectoryFileSystem saveDirFs = new(concatFs);
    ...
    private void ReadControls()
    {
        foreach (Title title in Titles.Values.Where(x => x.ControlNca != null))
        {
            ...
            using (UniqueRef<IFile> control = new())
            {
            ...
            int i = 0;
            bool nameSet = false;
            foreach (ApplicationControlProperty.ApplicationTitle desc in title.Control.Value.Title)
            {
...
public class Title
{
    ...
    public List<SwitchFsNca> Ncas { get; } = new();
...
public class Application
{
    ...
    public List<Title> AddOnContent { get; } = new();

    public ulong TitleId { get; private set; }
    public TitleVersion Version { get; private set; }
    public BlitStruct<ApplicationControlProperty> Nacp { get; private set; } = new(1);
