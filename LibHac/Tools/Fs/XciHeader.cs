...
public class XciHeader
{
    ...
    public XciHeader(KeySet keySet, Stream stream)
    {
        ...
        {
            using (BinaryReader r = new(keyAreaStorage.AsStream(), Encoding.Default, true))
            {
                HasInitialData = true;
        ...
        using (BinaryReader reader = new(bodyStorage.AsStream(), Encoding.Default, true))
        {
            ...
            if (keySet != null && !keySet.XciHeaderKey.IsZeros())
            {
                ...
                using (BinaryReader decReader = new(new MemoryStream(decHeader)))
                {
    ...
    private Result DecryptCardInitialData(Span<byte> dest, ReadOnlySpan<byte> initialData, int kekIndex, KeySet keySet)
    {
        ...
        using (AesCcm decryptor = new(key))
        {
            try
            {
