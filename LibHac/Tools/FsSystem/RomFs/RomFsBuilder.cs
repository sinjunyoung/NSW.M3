...
public class RomFsBuilder
{
    ...
    private List<IStorage> Sources { get; } = new();
    private HierarchicalRomFileTable<RomFileInfo> FileTable { get; } = new();
    ...
    public void AddFile(string path, IFile file)
    {
        ...
        NullStorage padding = new(CurrentOffset - newOffset);
        Sources.Add(padding);
    ...
    public IStorage Build()
    {
        ...
        BinaryWriter headerWriter = new(new MemoryStream(header));

        List<IStorage> sources = new();
