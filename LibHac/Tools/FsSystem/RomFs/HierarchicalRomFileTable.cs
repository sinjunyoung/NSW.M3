...
public class HierarchicalRomFileTable<T> where T : unmanaged
{
    ...
    private int _lastDirectoryOffset;
    private int _lastFileOffset;
    ...
    public void AddFile(string path, ref T fileInfo)
    {
        ...
        bool reUsePath = System.IO.Path.GetDirectoryName(path)!.Equals(System.IO.Path.GetDirectoryName(_lastFilePath));
    
        ReadOnlySpan<byte> utf8 = StringUtils.StringToUtf8(path);
        
        this.CreateFileRecursiveInternal(utf8, ref fileInfo, reUsePath);
    ...
    {
        RomEntryKey key = new(ReadOnlySpan<byte>.Empty, 0);
        DirectoryRomEntry entry = new();
        entry.NextSibling = -1;
    ...
    private void CreateFileRecursiveInternal(ReadOnlySpan<byte> path, ref T fileInfo, bool reUseDirectory)
    {
        int directoryOffset = 0;
        PathParser pathParser = new PathParser(path);
        RomEntryKey key = new RomEntryKey();
        pathParser.TryGetNext(out key.Name);
        if (reUseDirectory)
        {
            directoryOffset = _lastDirectoryOffset;
            key.Parent = _lastDirectoryOffset;
        ...
        {
            _lastDirectoryOffset = 0;
            while (!pathParser.IsFinished())
            {
                int newDirectoryOffset = DirectoryTable.GetOffsetFromKey(ref key);
                if (newDirectoryOffset < 0)
                {
                    ref DirectoryRomEntry newDirectory = ref DirectoryTable.AddOrGet(ref key, out newDirectoryOffset, out bool _, out Span<byte> _);
                    newDirectory.NextSibling = -1;
                    newDirectory.Pos.NextDirectory = -1;
                    newDirectory.Pos.NextFile = -1;
                    ref DirectoryRomEntry currentDirectory = ref DirectoryTable.GetValueReference(directoryOffset);
                    if (currentDirectory.Pos.NextDirectory == -1)
                    {
                        currentDirectory.Pos.NextDirectory = newDirectoryOffset;
                    }
                    else
                    {
                        ref DirectoryRomEntry nextDirectory = ref DirectoryTable.GetValueReference(currentDirectory.Pos.NextDirectory);
                        while (nextDirectory.NextSibling != -1)
                            nextDirectory = ref DirectoryTable.GetValueReference(nextDirectory.NextSibling);
                        nextDirectory.NextSibling = newDirectoryOffset;
                    }
                }
                directoryOffset = newDirectoryOffset;
                key.Parent = newDirectoryOffset;
                _lastDirectoryOffset = directoryOffset;
                pathParser.TryGetNext(out key.Name);
            }
        }

        ref FileRomEntry newFile = ref FileTable.AddOrGet(ref key, out int fileOffset, out bool alreadyExists, out _);
        newFile.Info = fileInfo;
        if (!alreadyExists)
            newFile.NextSibling = -1;

        if (reUseDirectory)
        {
            ref FileRomEntry lastFile = ref FileTable.GetValueReference(_lastFileOffset);
            
            int newFileOffset = fileOffset;
            
            while (newFile.NextSibling != -1)
            {
                newFileOffset = newFile.NextSibling;
                newFile = ref FileTable.GetValueReference(newFile.NextSibling);
            }
            
            lastFile.NextSibling = fileOffset;
            
            fileOffset = newFileOffset;
        }
        else
        {
            ref DirectoryRomEntry directory = ref DirectoryTable.GetValueReference(directoryOffset);
            if (directory.Pos.NextFile == -1)
            {
                directory.Pos.NextFile = fileOffset;
            }
            else
            {
                ref FileRomEntry nextFile = ref FileTable.GetValueReference(directory.Pos.NextFile);
                while (nextFile.NextSibling != -1)
                    nextFile = ref FileTable.GetValueReference(nextFile.NextSibling);
                nextFile.NextSibling = fileOffset;
            }
        }

        _lastFileOffset = fileOffset;
    }
