...
public class AesXtsFileSystem : IFileSystem
{
    ...
    public Result CreateFile(ref readonly Path path, long size, CreateFileOptions options, byte[] key)
    {
        ...
        AesXtsFileHeader header = new(key, size, path.ToString(), _kekSource, _validationKey);
    ...
    {
        using UniqueRef<IDirectory> baseDir = new();
    ...
    protected override Result DoOpenFile(ref UniqueRef<IFile> outFile, ref readonly Path path, OpenMode mode)
    {
        ...
        AesXtsFile xtsFile = new(mode, ref baseFile.Ref, new U8String(path.GetString().ToArray()), _kekSource,
