...
public class CachedStorage : IStorage
{
    ...
    private LinkedList<CacheBlock> Blocks { get; } = new();
    private Dictionary<long, LinkedListNode<CacheBlock>> BlockDict { get; } = new();
    ...
    public CachedStorage(IStorage baseStorage, int blockSize, int cacheSize, bool leaveOpen)
    {
        ...
        {
            CacheBlock block = new() { Buffer = new byte[blockSize], Index = -1 };
