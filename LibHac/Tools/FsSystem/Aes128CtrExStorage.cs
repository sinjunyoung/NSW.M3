...
public class Aes128CtrExStorage : Aes128CtrStorage
{
    ...
    private BucketTree Table { get; } = new();
        ...
        : base(baseStorage, key, counter, leaveOpen)
    {
        ...
        using ValueSubStorage valueNodeStorage = new(nodeStorage, 0, nodeStorageSize);
        using ValueSubStorage valueEntryStorage = new(entryStorage, 0, entryStorageSize);

        if (entryCount == 0)
        {
            Table.Initialize(NodeSize, 0);
            return;
        }
